...
class Diffusion_Predictor(object):
    def __init__(self, state_dim, action_dim, device, config, log_writer=False):
        ...
        self.action_grad_norm = self.action_dim
        self.ac_grad_norm = 2.0
    ...
    def action_gradient(self, replay_buffer,t_critic):
        ...
        action = a[:, self.condition_step - 1]
        # action梯度上升
        best_actions = torch.as_tensor(next_state.detach().cpu().numpy(), device=self.device)
        actions_optim = torch.optim.Adam([best_actions], lr=0.03, eps=1e-5)
        for i in range(self.action_gradient_steps):
            best_actions.requires_grad_(True)
            q1, q2 = self.critic(action, t_critic, pre_state_condition, best_actions)
    ...
    def train(self, replay_buffer, iterations, batch_size, log_writer=False):
        ...
        for _ in range(iterations):
            ...
            n_pre_state_condition = ns[:, 0:self.condition_step]
            n_next_state = ns[:, self.condition_step]
            n_action = a[:, self.condition_step]
            t_critic = torch.ones(action.shape[0]).to(self.device)

            r = r[:, self.condition_step]
            # q_learning
            current_q1, current_q2 = self.critic(action, t_critic, pre_state_condition, next_state)

            predic_state = self.ema_model(n_next_state, n_action, n_pre_state_condition, 1)
            target_q1, target_q2 = self.critic_target(n_action, t_critic, n_pre_state_condition, predic_state)
            target_q = torch.min(target_q1, target_q2)

            target_q = (r + target_q).detach()
            ...
            next_state, action, pre_state_condition = self.action_gradient(replay_buffer,t_critic)
            pred_loss, state_recon = self.predictor.loss(next_state, action, pre_state_condition, None, t, weights=1.0)
            ...
            for i in range(1, self.NonM_step):
                pre_state_condition = torch.cat([pre_state_condition[:, 1:], state_recon.reshape(batch_size, 1, -1)],
                                                dim=1)
                next_state = s[:, self.condition_step + i]
                action = a[:, self.condition_step - 1 + i]
                weights = torch.ones_like(state_recon)
                pred_loss_plus, state_recon = self.predictor.loss(next_state, action, pre_state_condition, None, t,
                                                                    weights=weights)
    ...
    def denoise_state(self, noise_next_state, current_action, condition_states, timestep,
        ...
        # final = copy.copy(noise_next_state)
        # noise_next_state = noise_next_state[:2, :]
    ...
    def demask_state(self, masked_next_state, action, states, mask, reverse_step=2):
        ...
        repeat = 50
        # final = copy.copy(noise_next_state)
        # masked_next_state = masked_next_state[:1, :]
        # action = action[:1,:]
        # states = states[:1,:,:]
        # mask = mask[:1,:]
    ...
    def save_model(self, file_name):
        ...
        torch.save({'actor_state_dict': self.predictor.state_dict(),
                    ...
                    'actor_optimizer_state_dict': self.predictor_optimizer.state_dict(),
