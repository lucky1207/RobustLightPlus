...
def parse_args():
    ...
    parser.add_argument("-memo",       type=str,           default='Advanced_Colight')
...
def main(in_args=None):
    ...
    for traffic_file in traffic_file_list:
        ...
        dic_traffic_env_conf_extra = {
            ...
            "NUM_ROW": NUM_ROW,
            "NUM_COL": NUM_COL,
            'diffusion_path': f'./checkpoints/unet_ad/{in_args.traffic_file}/diffusion_model_meta_final.pth',
            "TRAFFIC_FILE": traffic_file,
            "ROADNET_FILE": "roadnet_{0}.json".format(road_net),
            'is_test': True,
           #"inference_epoch": 50,
            'is_inference': in_args.inference,
            "sota_path": '../RobustLight/model/Advanced_Colight/' + traffic_file[:-5],
        ...
        dic_path_extra = {
            "PATH_TO_MODEL": os.path.join("model", in_args.memo, f'{in_args.traffic_file}_{in_args.inference}_{in_args.NOISE_SCALE}_{in_args.NOISE_TYPE}_{in_args.NOISE_LEVEL}_{in_args.NOISE_DIRECTION}.json_{time.strftime("%m_%d_%H_%M_%S", time.localtime(time.time()))}'),
            ...
            #                                        + time.strftime('%m_%d_%H_%M_%S', time.localtime(time.time()))),
            "PATH_TO_WORK_DIRECTORY": os.path.join("records", in_args.memo, f'{in_args.traffic_file}_{in_args.inference}_{in_args.NOISE_SCALE}_{in_args.NOISE_TYPE}_{in_args.NOISE_LEVEL}_{in_args.NOISE_DIRECTION}.json_{time.strftime("%m_%d_%H_%M_%S", time.localtime(time.time()))}'),
