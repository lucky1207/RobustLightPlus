...
class AdvancedMPLightAgent(NetworkAgent):
    """
    ...
    def build_network(self):
        ...
        _d = Dense(4, activation="sigmoid", name="num_vec_mapping")
        _d2 = Dense(4, activation="sigmoid", name="num_vec_mapping_part")
        dic_lane = {}
        dic_index = {
            "WL": 0,
            "WT": 1,
        ...
        for i, m in enumerate(self.dic_traffic_env_conf["list_lane_order"]):
            ...
            tmp_vec2 = _d2(
                Lambda(slice_tensor, arguments={"index": idx}, name="vec2_%d" % i)(dic_input_node["feat3"]))
            tmp_phase = Lambda(slice_tensor, arguments={"index": i}, name="phase_%d" % i)(_p)
            dic_lane[m] = concatenate([tmp_vec, tmp_phase, tmp_vec2], name="lane_%d" % i)

        list_phase_pressure = []
        if self.num_actions == 8 or self.num_actions == 4:

            lane_embedding = Dense(24, activation="relu", name="lane_embedding")
        ...
        if num_phase == 8:
            feature_map = Reshape((8, 7, 48))(list_phase_pressure_recomb)
        else:
            feature_map = Reshape((4, 3, 48))(list_phase_pressure_recomb)
    ...
    def choose_action(self, is_test, states):
        ...
        for s in states:
            for feature_name in self.dic_traffic_env_conf["LIST_STATE_FEATURE"]:
                if feature_name == "cur_phase":
                    dic_state_feature_arrays[feature_name].append(self.dic_traffic_env_conf['PHASE']
                                                                  [s[feature_name][0]])
                else:
                    dic_state_feature_arrays[feature_name].append(s[feature_name])
        state_input = [np.array(dic_state_feature_arrays[feature_name]) for feature_name in
        ...
        if is_test:
            if self.dic_traffic_env_conf['NOISE_LEVEL'] == 0:   
                if self.dic_traffic_env_conf['NOISE_TYPE'] == 2:
                    ...
                    for k in range(self.num_agents):
                        rpt_noised_action = self.q_network([rpt_state_phase[:,k,:].cpu().numpy(),rpt_noised_state[:,k,8:20].cpu().numpy(), rpt_noised_state[:,k,20:].cpu().numpy()])
                        # 输出action
                ...
                if self.dic_traffic_env_conf['NOISE_TYPE'] == 3:
                    ...
                    for k in range(self.num_agents):
                        rpt_noised_all_q = self.q_network([rpt_state_phase[:,k,:].cpu().numpy(),rpt_noised_state[:,k,8:20].cpu().numpy(), rpt_noised_state[:,k,20:].cpu().numpy()])
                        original_action_all_q = self.q_network([rpt_state_phase[:,k,:].cpu().numpy(), rpt_state[:,k,8:20].cpu().numpy(), rpt_state[:,k,20:].cpu().numpy()])
                        # 输出action
    ...
    def prepare_Xs_Y(self, memory):
        ...
        self.replay_buffer = ReplayBuffer((np.swapaxes(np.concatenate([_state[1],_state[2]],axis=1).reshape(-1, self.num_agents, 24),0,1), np.eye(4)[np.array(_action).reshape(self.num_agents,-1)], np.swapaxes(np.concatenate([_next_state2[1],_next_state2[2]],axis=1).reshape(-1, self.num_agents, 24),0,1), np.array(_reward).reshape(self.num_agents,-1)[:,:, np.newaxis]),
