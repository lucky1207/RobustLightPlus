...
class Intersection:
    ...
    def _update_feature(self):
        ...
        dic_feature["lane_enter_running_part"] = list(enter_running_part/20)
    ...
    def _get_part_traffic_movement_features(self):
        ...
        part_entering_running = (np.array(list_entering_num_l) - np.array(list_entering_part_queue))/self.max_vehicle
    ...
    def _get_pressure(self):
        return [self.dic_lane_waiting_vehicle_count_current_step[lane]/self.max_vehicle for lane in self.list_entering_lanes] + \
               [-self.dic_lane_waiting_vehicle_count_current_step[lane]/self.max_vehicle for lane in self.list_exiting_lanes]

    def _get_lane_queue_length(self, list_lanes):
        """
        queue length for each lane
        """
        return [self.dic_lane_waiting_vehicle_count_current_step[lane]/self.max_vehicle for lane in list_lanes]

    def _get_lane_num_vehicle_entring(self):
        """
        vehicle number for each lane
        """
        return [len(self.dic_lane_vehicle_current_step[lane])/self.max_vehicle for lane in self.list_entering_lanes]

    def _get_lane_num_vehicle_downstream(self):
        """
        vehicle number for each lane, exiting
        """
        return [len(self.dic_lane_vehicle_current_step[lane])/self.max_vehicle for lane in self.list_exiting_lanes]
...
class CityFlowEnv:
    ...
    def _inner_step(self, action):
        ...
        for i in range(int(1/self.dic_traffic_env_conf["INTERVAL"])):
            ...
            for v_id in vehicle_ids:
                ...
                if v_id not in self.vehicle_list:
                    self.vehicle_list[v_id] = 1
                if speed < 0.1:
    ...
    def get_avt(self):
        total_wait = sum([ item['time'] for item in list(self.waiting_vehicle_list.values())])
        return total_wait / len(list(self.waiting_vehicle_list.values()))
